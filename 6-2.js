...
function execute(order, area) {
	var points = [area[0].split(","), area[1].split(",")];
	...
	for (var i = points[0][0]; i <= points[1][0]; i++) {
		for (var j = points[0][1]; j <= points[1][1]; j++) {
			order(i, j);
		}
	}
}
...
for (var i = 0; i < input.length; i++) {
	...
	var area = [words[words.length - 3], words[words.length - 1]];
	execute(order, area);
}
...
console.log("The total brightness of all the lights is " + brightness + ".");